...
if [ ${CHK} -gt 0 ];then
#
C_FLAGS=$(FindPKG_CFLAGS json-c ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'json-c' not found." $?
#
LD_FLAGS=$(FindPKG_LDFLAGS json-c ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'json-c' not found." $?
#
