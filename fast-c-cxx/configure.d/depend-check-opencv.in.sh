...
if [ ${CHK} -gt 0 ];then
#
C_FLAGS=$(FindPKG_CFLAGS opencv4 ${THIRDPARTY_PREFIX} ${FAST_C_CXX_TARGET_MACHINE})
exit_if_error $? "'opencv|opencv4' not found." $?
#
LD_FLAGS=$(FindPKG_LDFLAGS opencv4 ${THIRDPARTY_PREFIX} ${FAST_C_CXX_TARGET_MACHINE})
exit_if_error $? "'opencv|opencv4' not found." $?
#
