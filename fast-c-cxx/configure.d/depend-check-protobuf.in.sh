...
if [ ${CHK} -gt 0 ];then
#
C_FLAGS=$(FindPKG_CFLAGS protobuf ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'protobuf' not found." $?
#
LD_FLAGS=$(FindPKG_LDFLAGS protobuf ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'protobuf' not found." $?
#
