...
if [ ${CHK} -gt 0 ];then
#
C_FLAGS=$(FindPKG_CFLAGS sqlite3 ${THIRDPARTY_PREFIX} ${FAST_C_CXX_TARGET_MACHINE})
exit_if_error $? "'sqlite3' not found." $?
#
LD_FLAGS=$(FindPKG_LDFLAGS sqlite3 ${THIRDPARTY_PREFIX} ${FAST_C_CXX_TARGET_MACHINE})
exit_if_error $? "'sqlite3' not found." $?
#
