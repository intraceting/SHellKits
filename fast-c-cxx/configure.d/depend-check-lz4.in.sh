...
if [ ${CHK} -gt 0 ];then
#
C_FLAGS=$(FindPKG_CFLAGS liblz4 ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'liblz4' not found." $?
#
LD_FLAGS=$(FindPKG_LDFLAGS liblz4 ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'liblz4' not found." $?
#
