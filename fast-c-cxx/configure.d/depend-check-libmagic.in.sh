...
if [ ${CHK} -gt 0 ];then
#
C_FLAGS=$(FindPKG_CFLAGS libmagic ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'libmagic|FILE' not found." $?
#
LD_FLAGS=$(FindPKG_LDFLAGS libmagic ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'libmagic|FILE' not found." $?
#
