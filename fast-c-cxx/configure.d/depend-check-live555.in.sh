...
if [ ${CHK} -gt 0 ];then
#
INC_PATH=$(FindINC_PATH "liveMedia/liveMedia.hh"  ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'live555|liveMedia' not found." $?
#
LIB_PATH=$(FindLIB_PATH liveMedia ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'live555|liveMedia' not found." $?
#
