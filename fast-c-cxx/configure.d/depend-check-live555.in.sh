...
if [ ${CHK} -gt 0 ];then
#
INC_PATH=$(FindINC_PATH "liveMedia/liveMedia.hh"  ${THIRDPARTY_PREFIX} ${FAST_C_CXX_TARGET_MACHINE})
exit_if_error $? "'live555|liveMedia' not found." $?
#
LIB_PATH=$(FindLIB_PATH liveMedia ${THIRDPARTY_PREFIX} ${FAST_C_CXX_TARGET_MACHINE})
exit_if_error $? "'live555|liveMedia' not found." $?
#
