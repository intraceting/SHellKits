...
if [ ${CHK} -gt 0 ];then
#
INC_PATH=$(FindINC_PATH "faiss/Index.h"  ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'faiss' not found." $?
#
LIB_PATH=$(FindLIB_PATH faiss ${THIRDPARTY_PREFIX} ${SHELLKITS_TARGET_MULTIARCH})
exit_if_error $? "'faiss' not found." $?
#
