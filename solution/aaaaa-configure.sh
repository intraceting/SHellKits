...
CompilerSelect()
{
    ${SHELLDIR}/../solution/compiler-configure.sh -d SOLUTION_PREFIX=AAAAA -d TARGET_COMPILER_PREFIX="$1"
}
...
# $4 TARGET_BITWIDE
{
    ${SHELLDIR}/../requires/find_package.sh -d PACKAGE_FLAG="$2" -d PACKAGE_NAME="$1" -d SYSROOT_PATH="$3" -d TARGET_BITWIDE="$4"
}
...
# $2 def
{
    ...
    {
        (DependCheck ${PACKAGE_KEY} 3 ${THIRDPARTY_SYSROOT} ${AAAAA_TARGET_BITWIDE})
        CHK=$?

        if [ ${CHK} -eq 0 ];then
        {
            THIRDPARTY_FLAGS="-D${PACKAGE_DEF} $(DependCheck ${PACKAGE_KEY} 2 ${THIRDPARTY_SYSROOT} ${AAAAA_TARGET_BITWIDE}) ${THIRDPARTY_FLAGS}"
            THIRDPARTY_LINKS="$(DependCheck ${PACKAGE_KEY} 3 ${THIRDPARTY_SYSROOT} ${AAAAA_TARGET_BITWIDE}) ${THIRDPARTY_LINKS}"
            THIRDPARTY_ENABLE+=("${PACKAGE_DEF}")
        }
        else
        {
            THIRDPARTY_NOTFOUND="$(DependCheck ${PACKAGE_KEY} 1 ${THIRDPARTY_SYSROOT} ${AAAAA_TARGET_BITWIDE}) ${THIRDPARTY_NOTFOUND}"
        }
        fi
...
#
COMPILER_CONF=$(CompilerSelect ${COMPILER_PREFIX})
...
#
CheckSTD c "${AAAAA_TARGET_COMPILER_C}" "${STD_C}"
if [ $? -ne 0 ];then
{
...
#
CheckSTD cxx "${AAAAA_TARGET_COMPILER_CXX}" "${STD_CXX}"
if [ $? -ne 0 ];then
{
...
{
    #
    CheckNVCCSTD ${CUDA_COMPILER_BIN} ${STD_CXX} "${AAAAA_TARGET_COMPILER_CXX}"
    if [ $? -ne 0 ];then
    {
...
#
DependHeaderCheck c "${AAAAA_TARGET_COMPILER_C}" ${STD_C} "libintl.h" HAVE_LIBINTL_H
DependHeaderCheck c "${AAAAA_TARGET_COMPILER_C}" ${STD_C} "pthread.h" HAVE_PTHREAD_H
DependHeaderCheck c "${AAAAA_TARGET_COMPILER_C}" ${STD_C} "iconv.h" HAVE_ICONV_H
DependHeaderCheck c "${AAAAA_TARGET_COMPILER_C}" ${STD_C} "linux/gpio.h" HAVE_GPIO_H
...
MSGCAT = ${MSGCAT_BIN}
#
CC = ${AAAAA_TARGET_COMPILER_C}
CXX = ${AAAAA_TARGET_COMPILER_CXX}
AR = ${AAAAA_TARGET_COMPILER_AR}
#
...
OPTIMIZE_LEVEL = ${OPTIMIZE_LEVEL}
#
TARGET_PLATFORM = ${AAAAA_TARGET_PLATFORM}
TARGET_ARCH = ${AAAAA_TARGET_ARCH}
TARGET_BITWIDE = ${AAAAA_TARGET_BITWIDE}
#
