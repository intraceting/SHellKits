...
{ 
    if [ ${FLAG} -eq 1 ];then
        exit $(CheckHavePackageFromKit "libonnxruntime-dev")
    elif [ ${FLAG} -eq 2 ];then
        PackageConfig  --cflags libonnxruntime 
    elif [ ${FLAG} -eq 3 ];then
        PackageConfig  --libs libonnxruntime
    elif [ ${FLAG} -eq 4 ];then
        echo "libonnxruntime-dev"
    else
        exit 22
    fi
}
...
{
    if [ ${FLAG} -eq 1 ];then
        exit $(CheckHavePackageFromKit "libonnxruntime-devel")
    elif [ ${FLAG} -eq 2 ];then
        PackageConfig  --cflags libonnxruntime 
    elif [ ${FLAG} -eq 3 ];then
        PackageConfig  --libs libonnxruntime
    elif [ ${FLAG} -eq 4 ];then
        echo "libonnxruntime-devel"
    else
        exit 22
    fi
}
