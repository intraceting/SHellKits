#!/bin/bash
#
# This file is part of AAUTH.
#  
# Copyright (c) 2025 The SHELLKITS project authors. All Rights Reserved.
# Copyright (c) 2024 The AAUTH project authors. All Rights Reserved.
...
{ 
    if [ ${FLAG} -eq 1 ];then
        exit $(CheckHavePackageFromKit "aauth-dev")
    elif [ ${FLAG} -eq 2 ];then
        PackageConfig  --cflags aauth 
    elif [ ${FLAG} -eq 3 ];then
        PackageConfig  --libs aauth 
    elif [ ${FLAG} -eq 4 ];then
        echo "aauth-dev"
    else
        exit 22
...
{
    if [ ${FLAG} -eq 1 ];then
        exit $(CheckHavePackageFromKit "aauth-devel")
    elif [ ${FLAG} -eq 2 ];then
        PackageConfig  --cflags aauth 
    elif [ ${FLAG} -eq 3 ];then
        PackageConfig  --libs aauth 
